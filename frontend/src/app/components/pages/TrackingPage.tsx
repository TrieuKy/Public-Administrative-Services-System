                 ...
                 <div className="bg-gray-50 border border-gray-200 rounded-lg p-6">
                    <p className="text-sm text-center text-gray-600 mb-4">
                      {searchResult.rating || isRatingSubmitted 
                        ? `Bạn đã đánh giá trải nghiệm về thủ tục và sự hỗ trợ của cán bộ ${searchResult.officer?.fullName || ''}.`
                        : `Vui lòng đánh giá trải nghiệm của bạn về thủ tục và sự hỗ trợ của cán bộ ${searchResult.officer?.fullName || ''}.`
                      }
                    </p>
                    <div className="flex justify-center gap-2 mb-6">
                      {[1, 2, 3, 4, 5].map((star) => {
                         const currentRating = searchResult.rating || ratingVal;
                         const isRated = searchResult.rating || isRatingSubmitted;
                         return (
                           <button 
                             key={star} 
                             onClick={() => !isRated && setRatingVal(star)}
                             disabled={isRated}
                             className={`p-2 transition ${currentRating >= star ? 'text-yellow-400' : 'text-gray-300'} ${!isRated && 'hover:text-yellow-200'}`}
                           >
                             <Star size={36} fill={currentRating >= star ? "currentColor" : "none"} />
                           </button>
                         )
                      })}
                    </div>
                    {searchResult.rating || isRatingSubmitted ? (
                      <div className="text-center">
                        <div className="mb-4 text-left p-4 bg-white border border-gray-200 rounded-lg text-sm text-gray-700 italic">
                          {searchResult.ratingContent || feedback || 'Không có nhận xét chi tiết'}
                        </div>
                        <Button disabled className="bg-gray-300 text-gray-700 font-medium px-8 w-full md:w-auto cursor-not-allowed">
                          <CheckCircle className="mr-2 h-4 w-4" /> Đã đánh giá ({searchResult.rating || ratingVal} sao)
                        </Button>
                      </div>
                    ) : (
                      <>
                        <textarea
                          ...
                          placeholder="Để lại góp ý chi tiết (Không bắt buộc)..."
