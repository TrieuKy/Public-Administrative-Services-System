...
const PERIODS = [
  ...
  { key: 'q1',     label: 'Quý 1/2026' },
  { key: 'q2',     label: 'Quý 2/2026' },
  { key: 'q3',     label: 'Quý 3/2026' },
  { key: 'q4',     label: 'Quý 4/2026' },
...
export function OfficerReports() {
  ...
  const fetchReports = async (p: string) => {
    setLoading(true);
    try {
      const res = await axiosInstance.get(`/officer/dashboard/reports?period=${p}`);
  ...
  const handleExport = () => {
    ...
    // Tạo nội dung CSV
    const rows = [
      ['Kỳ báo cáo', PERIODS.find(p => p.key === period)?.label || period],
      ['Tổng hồ sơ tiếp nhận', data.totalReceived],
      ['Tỷ lệ hoàn thành đúng hạn (%)', data.onTimeRate],
      ['Thời gian xử lý trung bình (ngày)', data.avgProcessingDays],
      ['Tỷ lệ từ chối (%)', data.rejectionRate],
      [],
      ['Tháng', 'Tiếp nhận', 'Hoàn thành', 'Từ chối'],
      ...(data.monthlyData || []).map((m: any) => [m.name, m.total, m.completed, m.rejected]),
    ];
    ...
    a.href = url;
    a.download = `BaoCaoCongViec_${period}_${new Date().toLocaleDateString('vi-VN').replace(/\//g, '-')}.csv`;
  ...
  if (!data) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-center text-gray-400">
        <div className="animate-spin w-8 h-8 border-4 border-red-200 border-t-red-600 rounded-full mx-auto mb-3" />
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap justify-between items-start gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
            {PERIODS.map(p => (
              <button
                ...
                disabled={loading}
                className={`px-3 py-2 text-xs font-medium border-r last:border-0 transition ${
                  period === p.key
          ...
          <Button onClick={handleExport} variant="outline" className="bg-white border-gray-300 shadow-sm text-sm">
      ...
      {loading && (
        <div className="text-center py-4 text-sm text-gray-500 animate-pulse">Đang tải dữ liệu cho kỳ đã chọn...</div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="p-5 border-l-4 border-l-blue-500 rounded-xl shadow-sm">
          <div className="flex justify-between items-start">
            ...
            <span className="text-xs font-semibold text-gray-400 bg-gray-100 px-2 py-1 rounded">{PERIODS.find(p => p.key === period)?.label}</span>
          </div>
          <div className="mt-4">
        ...
        <Card className="p-5 border-l-4 border-l-green-500 rounded-xl shadow-sm">
          <div className="flex justify-between items-start">
            ...
            <span className="text-xs font-semibold text-green-600 bg-green-50 px-2 py-1 rounded">Đúng hạn</span>
          </div>
          <div className="mt-4">
        ...
        <Card className="p-5 border-l-4 border-l-yellow-500 rounded-xl shadow-sm">
          <div className="flex justify-between items-start">
            ...
            <span className="text-xs font-semibold text-yellow-600 bg-yellow-50 px-2 py-1 rounded">TB</span>
          </div>
          <div className="mt-4">
        ...
        <Card className="p-5 border-l-4 border-l-red-500 rounded-xl shadow-sm">
          <div className="flex justify-between items-start">
            ...
            <span className="text-xs font-semibold text-red-600 bg-red-50 px-2 py-1 rounded">Từ chối</span>
          </div>
          <div className="mt-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-2 p-5 shadow-sm">
          <div className="mb-6">
            ...
            <p className="text-sm text-gray-500">Tiếp nhận, hoàn thành và từ chối — {PERIODS.find(p => p.key === period)?.label}</p>
          </div>
          <div className="h-72">
